...
**Vercel is possible, but it should be treated as a runtime adaptation rather than a drop-in rehost.** The first deployment symptom was diagnostic: the root URL returned the bundled `dist/index.js` with `content-type: application/javascript`, which exposed the Express server bundle as the page. The Vite client is built at `dist/public/index.html`. The repository now includes `vercel.json` with `framework: vite`, `buildCommand: pnpm build`, and `outputDirectory: dist/public`, so Vercel should serve the client HTML at `/` instead of the server bundle. The project still depends on Manus-specific auth, database, storage, and API environment variables, so the full-stack runtime requires a separate Vercel Function adaptation before OAuth and `/api/trpc` are expected to work. Vercel’s Express runtime does not serve static assets through `express.static()`; static files must be exposed through Vercel’s static output handling.[^1]
...
The current project should not be assumed to be production-ready on Vercel without a deployment rehearsal. The static-root issue is addressed by the checked-in `vercel.json` output configuration. A broad SPA rewrite is intentionally not included because it could redirect future `/api/*` requests to the homepage. The remaining production work is to adapt `server/_core/index.ts` into a Vercel-compatible function entrypoint for OAuth, `/api/trpc`, and storage routes, then update OAuth callback URLs to the Vercel domain and confirm that the database allows connections from the deployed runtime.
